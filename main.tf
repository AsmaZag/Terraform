...
resource "aws_volume_attachment" "ebsattach" {
	...
	provisioner "remote-exec" {
		inline = [
			...
			"sudo git clone https://github.com/AsmaZag/page-php.git /var/www/html/" // Downloading Files
		]
	}
...
resource "aws_s3_bucket_policy" "tfbucket_policy" {
  bucket = aws_s3_bucket.tfbucket.bucket
  policy = <<-POLICY
    {
      "Version": "2012-10-17",
      "Statement": [
        {
          ...
          "Principal": "*",
          "Action": "s3:GetObject",
          "Resource": "${aws_s3_bucket.tfbucket.arn}/Profile.jpg"
        }
      ]
    }
  POLICY
...
resource "null_resource" "tfModify" {
	...
	provisioner "remote-exec" {
		inline = [
      		"echo '<div style=\"text-align: center;\">' | sudo tee -a /var/www/html/index.php",
      		"echo '  <img class=\"image\" src=\"https://${aws_cloudfront_distribution.tfdistribution.domain_name}/Profile.jpg\" width=\"425\" height=\"285\" alt=\"Profile Image\">' | sudo tee -a /var/www/html/index.php",
      		"echo '</div>' | sudo tee -a /var/www/html/index.php"
    	]
	}
	depends_on = [
